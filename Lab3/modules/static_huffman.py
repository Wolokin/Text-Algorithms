...
class StaticCompressor:
    ...
    def encode(self, text, display_tree=False):
        ...
        bits[:3] - trailing zeros count (automatically appended zeros at the end of file)
        then the tree is encoded followed by a single 0 bit, then the text in huffman code
        followed by 0-7 trailing zeros. Something like this:
        [trailing_zeros, tree, '0', text]'''
        ...
        bits = bitarray()
        ascii_to_bin = dict()
        header_bit_count = [self.TRAILING_ZEROS_SIZE]
        ...
        tmp = bitarray()
        length = (len(bits) + self.TRAILING_ZEROS_SIZE)
        tmp.extend(int2ba((8 - (length % 8)) % 8,
    ...
    def decode(self):
        ...
        def tree_decoder():
            stack = []
            i = self.TRAILING_ZEROS_SIZE
            while bits[i] is not False or len(stack) != 1:
                if bits[i]:
                    i += 1
        ...
        with open(self.filename, "rb") as f:
            bits.fromfile(f)
        trailing_zeros_count = ba2int(bits[:self.TRAILING_ZEROS_SIZE])
        node, header_bit_count = tree_decoder()
        root = node
...
if __name__ == '__main__':
    text = "abracadabra"
    S = StaticCompressor('compressed.static', StaticHuffman)
    S.encode(text, True)
    print(S.decode())
